...
public class SchedulesRepository : GenericRepository<SchedulesEntity>, ISchedulesRepository
{
    ...
    public async Task<List<SchedulesEntity>?> GetScheduleByPurchaseId(Guid purchaseId)
    {
        return await _context.Schedules.Where(p => p.PurchaseId == purchaseId).ToListAsync();
    }
}